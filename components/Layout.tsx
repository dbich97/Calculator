...
const Layout: React.FC = () => {
    ...
    const dropdownLinks = (
        <>
            <NavLink to={`/${currentLang}/pregnancy-calculator`} className={({ isActive }) => `${navLinkClasses} block w-full text-left ${isActive ? activeNavLinkClasses : ''}`}>{t.navPregnancyCalculator}</NavLink>
                        ...
                        <div className="flex items-center">
                            ...
                            <div className="hidden md:block">
                                <div className="ml-10 flex items-baseline space-x-4">
                                    ...
                                    <NavLink to={`/${currentLang}/calculator`} className={({ isActive }) => `${navLinkClasses} ${isActive ? activeNavLinkClasses : ''}`}>{t.navCalculator}</NavLink>
                                    <div className="relative" ref={desktopDropdownRef}>
                                        <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className={`${navLinkClasses} flex items-center`}>
                ...
                {isMobileMenuOpen && (
                    <div className="md:hidden">
                        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                            ...
                            <NavLink to={`/${currentLang}/calculator`} className={({ isActive }) => `block ${navLinkClasses} ${isActive ? activeNavLinkClasses : ''}`}>{t.navCalculator}</NavLink>
                            <div className="relative" ref={mobileDropdownRef}>
                                <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className={`w-full text-left flex items-center justify-between ${navLinkClasses}`}>
                                    {t.navHealthCalculators}
                                    <svg className={`ml-1 h-5 w-5 transform transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                                </button>
                                {isDropdownOpen && (
            ...
            <footer className="bg-white dark:bg-gray-800 shadow-inner mt-auto">
                <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row justify-between items-center text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <div className="flex space-x-4 mt-2 sm:mt-0">
                        <Link to={`/${currentLang}/privacy`} className="hover:text-blue-500">{t.footerPrivacyPolicy}</Link>
                        <Link to={`/${currentLang}/about`} className="hover:text-blue-500">{t.navAbout}</Link>
                        <Link to={`/${currentLang}/contact`} className="hover:text-blue-500">{t.footerContactUs}</Link>
                    </div>
                </div>
            </footer>
        </div>
